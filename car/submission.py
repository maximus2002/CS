...
class ExactInference(object):
    ...
    def elapseTime(self):
        ...
        for i in range(self.belief.getNumRows()):
            for j in range(self.belief.getNumCols()):
                s[(i,j)] = self.belief.getProb(i,j)
                self.belief.setProb(i,j,0)
        for key,value in self.transProb.items():
            self.belief.addProb(key[1][0],key[1][1],value*s[key[0]])
        self.belief.normalize()
        # END_YOUR_CODE
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        def distance(x1,y1,x2,y2):
            return (float(x1-y1)**2 + float(x2-y2)**2)**0.5
        pw = collections.defaultdict(float)
        for particle in self.particles:
            dist = distance(agentX,util.colToX(particle[1]),agentY,util.rowToY(particle[0]))
            pdf = util.pdf(dist, Const.SONAR_STD, observedDist) 
            pw[particle] = self.particles[particle]*pdf
        self.particles = collections.defaultdict(int)
        for i in range(self.NUM_PARTICLES):
            self.particles[util.weightedRandomChoice(pw)] += 1     
        # END_YOUR_CODE
    ...
    def elapseTime(self):
        ...
        for particle in self.particles:
            for i in range(self.particles[particle]):
                pw[util.weightedRandomChoice(self.transProbDict[particle])]+=1
        self.particles = pw
        self.updateBelief()
        # END_YOUR_CODE
