...
def create_chain_csp(n):
    ...
    for v in range(1,n):
        csp.add_binary_factor(variables[v],variables[v-1],xor)
    # END_YOUR_CODE
    return csp
...
def create_nqueens_csp(n = 8):
    ...
    #def binary(i,j):
        #csp.add_binary_factor(i,j,CD)

    domain = []
    for i in range(n):
        smallDomain = []
        for j in range(n):
            smallDomain.append((i,j))
        domain.append(smallDomain)
    ...
    for i in range(n):
        for j in range(n):
            if i != j:
                csp.add_binary_factor(variables[i],variables[j],CD) 
    # END_YOUR_CODE
    return csp
            ...
            for var in self.csp.variables:
                if var not in assignment:
                    ...
                    for value in self.domains[var]:
                        val += self.get_delta_weight(assignment, var, value)
                    list.append((var,val))
    ...
    for i in range(len(variables)):
        ...
        if i == 0:
            ...
            if xd != None:
                for items in xd:
                    ad.append((0,items))
            csp.add_variable(a,ad)
            csp.add_binary_factor(a,x,potential)
            amd = ad
        else:
            ad = []
            if xd != None:
                for items in xd:
                    if amd != None:
                        for item in amd:
                            ad.append((item[1],item[1]+items))
            ad = list(set(ad))
            amd = ad
            csp.add_variable(a,ad)
            csp.add_binary_factor(a,x,potential)
            csp.add_binary_factor(('sum',name,i - 1),a,lambda x,a:x[1] == a[0])
    csp.add_binary_factor(a,result,lambda x,a: a == x[1])
    return result
                ...
                if req.quarters != []:
                    if quarter not in req.quarters:
                        csp.add_unary_factor((req, quarter),\
                            lambda x: x is None or quarter in req.quarters)
        # END_YOUR_CODE
        ...
        for quarter in self.profile.quarters:
            ...
            for request in self.profile.requests:
                ...
                for course in courses:
                    maxUnit = self.bulletin.courses[course].maxUnits
                    minUnit = self.bulletin.courses[course].minUnits
                    domain  = range(minUnit,maxUnit+1)
                    domain.append(0)
                    csp.add_variable((course,quarter),domain)
                    csp.add_binary_factor((request,quarter),(course,quarter),Validate)
                    variables.append((course,quarter))
            quarterly_units = get_sum_variable(csp,quarter,variables,self.profile.maxUnits)
            csp.add_unary_factor(quarterly_units,\
                    lambda x: self.profile.minUnits <= x <= self.profile.maxUnits)
        # END_YOUR_CODE
