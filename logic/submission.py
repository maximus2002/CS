...
def formula2d():
    ...
    Mother = Parent('$z', '$y')
    #Grandparent = And(Parent('$x','$z'),Mother)
    GrandMom = And(Female('$y'),And(Parent('$x','$z'),Mother))
    ExistsGrandMom = Exists('$z',GrandMom)
    return Forall('$x', Forall('$y', Equiv(Grandmother('$x', '$y'), ExistsGrandMom)))
    # END_YOUR_CODE
...
def liar():
    ...
    formulas.append(Exists('$x', And(TellTruth('$x'), Forall('$y', Equiv(Not(Equals('$x','$y')),Not(TellTruth('$y')))))))
    formulas.append(Exists('$x', And(CrashedServer('$x'), Forall('$y', Equiv(Not(Equals('$x','$y')),Not(CrashedServer('$y')))))))
    # END_YOUR_CODE
...
def ints():
    ...
    #formulas.append(Forall('$x', Exists('$y',And(Successor('$x','$y'),And(Not(Equals('$x','$y')),Forall('$z',Equiv(And(successor('$x','$z'),Not(Equals('$x','$z'))),Equals('$y','$z'))))))))
    a = Equiv(And(Successor('$x', '$y'),Not(Equals('$x', '$y'))),Equals('$z', '$y'))
    b = [Successor('$x', '$z'),Not(Equals('$x', '$z')),Forall('$y',a)]
    formulas.append(Forall('$x', Exists('$z',AndList(b))))
